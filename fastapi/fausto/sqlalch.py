    ...
    def decorator(function):
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                return {
                    "error": True,
                    "msg": "SQLAlchemyError: Hay problemas con la integridad de datos, revisa los logs",
                    "in_error": True,
                }
    ...
    for k, v in dictionary_filter.items():
        if k in model.__dict__:
            ...
            if isinstance(v, list) and len(v) > 1:
                ...
                filters.append(model.__dict__[k] is None)
            elif isinstance(v, str) and re.search(r"<>", v):
                filters.append(
                    model.__dict__[k].between(v.split("<>")[0], v.split("<>")[1])
                )
            elif isinstance(v, str) and re.search(r"<=", v):
                filters.append(
                    model.__dict__[k] <= re.sub(r"<=", "", dictionary_filter[k])
                )
            elif isinstance(v, str) and re.search(r"<", v):
                filters.append(
                    model.__dict__[k] < re.sub(r"<", "", dictionary_filter[k])
...
def filter_fields_sqlalch_old(dictionary_object, fields):
