...
from auth.model.models import Permission, Role, RolePermission
...
def update_role(s, id, data):
    ...
    duplicate_role = (
        s.query(Role).filter(Role.name == data.get("name"), Role.id != id).one_or_none()
    )
    if duplicate_role:
        raise ControllerError("the role already exists: '" + data.get("name") + "'")
    ...
    permission_db = (
        ...
        .filter(RolePermission.id_role == id)
        .all()
    )
    ...
    for p in list_permission_db:
        if p not in permissions_received:
            logging.debug("SQLALCH state: " + str(p))
            s.query(RolePermission).filter(
                RolePermission.id_role == id, RolePermission.id_permission == p
            ).delete()
            s.commit()

    for lp in permissions_received:
        if lp not in list_permission_db:
            role_permission_format = {}
...
def delete_role(s, id):
    role_permission = (
        s.query(RolePermission).filter(RolePermission.id_role == id).delete()
    )
