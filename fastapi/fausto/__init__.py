...
def validate_required_data(
    json_dict: dict = {}, keys: dict = {}, optional_keys: dict = {}
):
    ...
    try:
        faltantes = [
            ...
            or json_dict.get(x) is None
            or json_dict.get(x) == ""
        ]
        sobrantes = [
            x for x in json_dict.keys() if x not in keys and x not in optional_keys
        ]
    ...
    return ValidateResult(
        ok=ok, msg=msg, data={"required": faltantes, "leftover": sobrantes}
    )


def validate_required_args(
    json_dict: dict = {}, keys: dict = {}, required: bool = False
):
    ...
    return ValidateResult(
        ok=ok,
        msg=msg,
        data={"required": faltantes, "leftover": sobrantes},
        filtered=filtrados,
    )
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            if size_error == 1:
                raise HTTPException(
                    status_code=500,
                    detail={"msg": str(error), "data": None, "error": True},
                )
            elif size_error > 1:
                raise HTTPException(
                    status_code=500,
