...
class createRole_ extends Component {
    ...
    getPermissions = () => {
        fotchAuth.get("/permission", obj => {
            ...
            if (!obj.error) {
                ...
                this.props.notierror("No permissions found")
            }
    ...
    getRoles = () => {
        const notiloading = this.props.notiloading("Getting roles...")
    ...
    postRole = () => {
        if (this.state.role_name === "" || this.state.role_display_name === "") {
            this.props.notiwarn("Fill the empty fields")
        } else if (this.state.permissions.length < 1) {
            this.props.notiwarn("Select permissions")
        }
        else {
    ...
    getRolePermissions = (rol) => {
        fotchAuth.get("/permission?role=" + rol, obj => {
            ...
            if (!obj.error) {
                ...
                this.props.notiwarn("No objects found!")
            }
    ...
    deleteRole = roleId => {
        console.log(roleId)
        const notiloading = this.props.notiloading("Deleting Role...")
...
const createRole = createRole_
const connectedComponent = connect(null, mapDispatchToProps)(createRole)
