...
def create_permission_type(s, data):
    duplicate_permission_type = (
        s.query(PermissionType).filter_by(name=data.get("name")).one_or_none()
    )
    if duplicate_permission_type:
        raise ControllerError(
            "the permission type already exists: '" + data.get("name") + "'"
        )
...
def update_permission_type(s, id, data):
    ...
    duplicate_permission_type = (
        s.query(PermissionType)
        .filter(PermissionType.name == data.get("name"), PermissionType.id != id)
        .one_or_none()
    )
    if duplicate_permission_type:
        raise ControllerError(
...
def get_permission_type(s, id):
    ...
    if permission_type:
        logging.debug(
            "SQLALCH PermissionType: " + str(quick_format_sqlalch(permission_type))
        )
