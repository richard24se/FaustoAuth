...
class AuditTypeService:
    ...
    async def create_audit_type(s: AsyncSession, *, data: AuditTypeCreate) -> dict:
        ...
        *,
        ...
        if data.name and data.name != audit_type.name:
            result = await s.execute(
                select(AuditType).filter(AuditType.name == data.name, AuditType.id != audit_type_id)
            )
            existing = result.scalars().first()
            if existing:
                raise ControllerError(f"The audit type '{data.name}' already exists.")
    ...
    @fapi_wrapper
    @async_sqlalch_wrapper
    async def delete_audit_type(s: AsyncSession, *, audit_type_id: int) -> dict:
        """Deletes an audit type."""
        result = await s.execute(select(AuditType).filter(AuditType.id == audit_type_id))
        audit_type = result.scalars().first()
        if not audit_type:
            raise ControllerError("Audit type not found, it may have already been deleted.", status_code=404)
    ...
    @fapi_wrapper
    @async_sqlalch_wrapper
    async def get_audit_type(s: AsyncSession, *, audit_type_id: int) -> dict[str, Any]:
