...
async def validate_user(  # Make validate_user async
    ...
    if not password_verified:
        ...
        logging.warning("Password verification failed for user '%s'. Attempting plain-text comparison.", username)
        if isinstance(user.password, str) and user.password == password:
            password_verified = True
            # Immediately re-hash and update the password for security
            logging.info("Plain-text password matched for user '%s'. Re-hashing and updating password.", username)
            ...
            truncated_password_bytes = password_bytes[:72]  # Keep truncation for safety/consistency
