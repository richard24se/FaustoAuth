...
async def list_audit_types(s: AsyncSession = Depends(get_async_db)):
...
@router.get(
    "/{audit_type_id}", response_model=Response, summary="Get an audit type by ID"
)
...
@router.put(
    "/{audit_type_id}", response_model=Response, summary="Update an audit type"
)
